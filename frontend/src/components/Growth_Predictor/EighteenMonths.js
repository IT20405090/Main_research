...
function Age18Months() {
  ...
  useEffect(() => {
    // Fetch data for 18 months from the Flask API endpoint
    fetch(`http://localhost:5000/get_milestone?age=18%20months`)
  ...
  const calculateScore = (section) => {
    const selectedCheckboxes = document.querySelectorAll(`input[name=${section}]:checked`);
    const totalCheckboxes = document.querySelectorAll(`input[name=${section}]`);
    const percentage = (selectedCheckboxes.length / totalCheckboxes.length) * 100;
  ...
  return (
    <div style={{ display: 'flex' }}>
      ...
      >
        <h1>Development Milestones - 18 Months</h1>
  ...
  <p>Loading data...</p>
      ...
      <div
        style={{
          flex: 1,
          background: 'rgba(173, 216, 230, 0.5)', // Set a white background
          padding: '1em',
          display: 'flex',
